"""Closed-loop eval — joint-only obs (53-d) and joint-only action (53-d).

Same as ``eval_lingbot_isaac_a2_joint_only.py`` but observation.state drops
the 14 EE pose dims, sending the 53 raw env joint positions only. The policy
server pads to its max_state_dim internally.

Usage::

    isaaclab.sh -p scripts/imitation_learning/eval_lingbot_isaac_a2_joint_only_state.py \\
...
parser = argparse.ArgumentParser(description="LingBot-VLA joint-only state+action eval inside Isaac Lab.")
...
parser.add_argument("--debug_dir", type=str, default="/tmp/lingbot_isaac_eval_joint_only_state")
...
def encode_obs_for_policy(env, task: str) -> dict:
    ...
    assert joint_pos.shape == (53,), f"got {joint_pos.shape}"
...
def main():
    ...
    for ep in range(args_cli.episodes):
        try:
            policy.reset("isaac_a2_joint_only_state")
